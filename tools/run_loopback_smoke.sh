...
run_client() {
	...
	client_gfx=0
	client_gfx_option=
	if [ "$case_name" = "gfx" ]; then
		client_gfx=1
		client_gfx_option=/gfx:AVC420
	fi

	if MACRDP_LOOPBACK_DURATION_MS="$duration_ms" \
		...
		"$client" \
			...
			/cert:ignore \
			/log-level:WARN \
			${client_gfx_option:+"$client_gfx_option"} >"$client_log" 2>&1; then
		client_status=0
	else
		client_status=$?
	fi
	...
	if [ "${max_interval:-0}" -gt "$max_interval_ms" ]; then
	...
	if [ "$case_name" = "gfx" ]; then
		if [ "${gfx_frames:-0}" -lt 1 ] || [ "${gfx_wire:-0}" -lt 1 ]; then
			fail "gfx client received no RDPGFX surface frames"
		fi
		if [ "${gfx_avc420:-0}" -lt 1 ]; then
			fail "gfx client did not receive AVC420 frames"
		fi
	else
		if [ "${gfx_frames:-0}" -ne 0 ] || [ "${gfx_wire:-0}" -ne 0 ]; then
			fail "nogfx client unexpectedly received RDPGFX frames"
		fi
	fi
}
