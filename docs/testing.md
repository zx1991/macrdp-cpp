...
counters. `client_queue_last`, `client_queue_max`, and `client_queue_threshold`
are the latest and maximum client-reported unprocessed graphics bytes and the
current adaptive demotion threshold; zero means the protocol made queue depth
unavailable and is not counted as a report. `recent_fps` and
